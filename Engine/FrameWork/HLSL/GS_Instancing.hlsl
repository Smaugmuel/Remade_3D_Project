...
cbuffer PerCallBuffer : register(b1)
{
	...
	int4 amounts;
	// x : The start index of matrix to be read from array
...
#endif
#ifdef CBUFFER_VIEW_PROJECTION_MATRIX
		outputs[0].position = mul(output[0].position, viewProjectionMatrix);
		outputs[1].position = mul(output[1].position, viewProjectionMatrix);
		outputs[2].position = mul(output[2].position, viewProjectionMatrix);
#endif
#ifdef VBUFFER_NORMAL
